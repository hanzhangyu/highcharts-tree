...
var Tree = /** @class */ (function () {
    function Tree(treeNode, horizontal) {
        if (horizontal === void 0) { horizontal = true; }
        this.horizontal = horizontal;
        this.treeMod = 0;
        this.root = this.buildTree(treeNode);
    }
    Tree.getTree = function (_a) {
        var tree = _a.tree, data = _a.data, horizontal = _a.horizontal;
        return tree && tree.root ? tree : new Tree(data, horizontal).build();
    };
    Tree.rotate = function (node) {
        var _a, _b;
        _a = __read([node.y, node.x], 2), node.x = _a[0], node.y = _a[1];
        _b = __read([node.height, node.width], 2), node.width = _b[0], node.height = _b[1];
        if (node.children)
            node.children.forEach(Tree.rotate);
    };
